...
const InventorySalesDetails = () => {
    ...
    const tableData = companySalesData.filter(data => data.action === 'Reduce')
    ...
    const clickHandler = (e) => {
        ...
        const newtalbleData = companySalesData.filter(data => {
            return data.action === 'Reduce' && data.date.slice(0, 10) >= startDate && data.date.slice(0, 10) <= endDate;
        });
        setTableDataInPeriod(newtalbleData);
    }
